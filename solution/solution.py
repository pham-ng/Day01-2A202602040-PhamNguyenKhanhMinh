...
def compare_models(prompt: str) -> dict:
    ...
    word_count = len(gpt4o_answer.split())
    gpt4o_cost = (word_count / 0.75) / 1000 * pricing["output"]
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    import tiktoken
    try:
        enc=tiktoken.ecoding_for_model(model)
        return len(enc.encode(text))
    except Exception:
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    pricing = PRICING_PER_1K_TOKENS.get(
        model, PRICING_PER_1K_TOKENS.get("gpt-4o", {"input": 0.0025, "output": 0.010})
    )
...
def run_assistant(
    ...
    while True:
        ...
        turns += 1
        cost_info = estimate_cost(user_msg, full_reply, model=OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for res in results:
        ...
        prompt = shorten(res.get("prompt", ""))
        gpt4o_resp = shorten(res.get("gpt4o_response", res.get("gpt4o_answer", "")))
        mini_resp = shorten(res.get("mini_response", res.get("mini_answer", "")))

        gpt4o_lat = res.get("gpt4o_latency", res.get("gpt4o_time", 0.0))
        mini_lat = res.get("mini_latency", res.get("mini_time", 0.0))

        
        row = f"{prompt} | {gpt4o_resp} | {mini_resp} | {gpt4o_lat:.2f}s | {mini_lat:.2f}s"
        rows.append(row)
